...
import { testRuntime } from "../support/plugins.js";
...
describe("persistence restore", () => {
  it("restores through governed replay with an uninterrupted-equivalent snapshot", () => {
    ...
    if (!loaded.ok || !loaded.value)
      throw new Error("Expected a stored event log");
    const restored = createSession(testRuntime, schema, loaded.value);
  ...
  it("rejects schema mismatch and corrupt ordering after a valid store load", () => {
    ...
    expect(createSession(testRuntime, changed, session.getEvents())).toEqual({
      ok: false,
      error: { code: "schema-mismatch" },
    });
    expect(
      createSession(testRuntime, schema, [
        ...session.getEvents(),
        ...session.getEvents(),
      ]),
    ).toEqual({
