...
describe("presentation mapping", () => {
  ...
  it("extracts question ids only from string problem locations", () => {
    expect(problemQuestion({ code: "required", where: { q: "name" } })).toBe(
      "name",
    );
    expect(
      problemQuestion({ code: "no-edge", where: { node: "page" } }),
    ).toBeUndefined();
    expect(
      problemQuestion({ code: "required", where: { q: 4 } }),
    ).toBeUndefined();
  });
  ...
  it("orders question problems by visible order and leaves page problems last", () => {
    ...
    expect(problemsForQuestion(problems, age)).toEqual([
      problems[1],
      problems[3],
    ]);
    ...
    expect(orderProblems([unknown, pageProblem], [name, age])).toEqual([
      unknown,
      pageProblem,
    ]);
  });
