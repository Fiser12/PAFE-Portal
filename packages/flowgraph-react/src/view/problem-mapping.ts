...
export const problemsForQuestion = (
  ...
  problems.filter((problem) => problemQuestion(problem) === question);
...
export const orderProblems = (
  ...
  return problems
    ...
    .sort((left, right) => {
      const leftOrder =
        order.get(problemQuestion(left.problem) ?? toQuestionId("")) ??
        Infinity;
      const rightOrder =
        order.get(problemQuestion(right.problem) ?? toQuestionId("")) ??
        Infinity;
