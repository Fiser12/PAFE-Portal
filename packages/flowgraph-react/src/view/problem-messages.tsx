...
const problemCopy: Readonly<Record<string, string>> = {
  ...
  "unsupported-event-version":
    "La versión del progreso guardado no es compatible.",
  "reentrant-dispatch": "Espera a que termine la acción actual.",
};

export const messageForProblem = (problem: Problem): string =>
  problemCopy[problem.code] ?? `Error de validación: ${problem.code}`;
    ...
    >
      <ul>
        {problems.map((problem, index) => (
          <li key={`${problem.code}-${String(index)}`}>
