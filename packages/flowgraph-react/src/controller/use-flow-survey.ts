...
import { deriveFlowView } from "../hooks/use-flow-view";
import type {
  AnswerResult,
...
import {
  clearQuestionFriction,
  emptyFriction,
  rejectedFriction,
} from "./friction";
import {
  draftRegistrySymbol,
  type InternalFlowSurveyController,
} from "./internal";
...
export const useFlowSurvey = ({
  ...
  const answer = useCallback<FlowSurveyController["answer"]>(
    (question, value) => {
      const result = dispatch(
        { kind: "ANSWER", q: question, value, meta: createMeta() },
        "answer",
      );
      if (result.ok) {
  ...
  const navigate = useCallback(
    (kind: "NEXT" | "BACK", action: "next" | "back"): AnswerResult => {
      ...
      return navigated.ok
        ? ok([...flushed.value, ...navigated.value])
        : navigated;
    },
