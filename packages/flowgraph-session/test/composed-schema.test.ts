...
const reportPack = (): FlowPack => {
  ...
  return {
    ...
    outlets: [
      { id: donePort, from: page, when: { kind: "always" }, required: true },
    ],
  };
};
...
const command = (schema: FlowSchema, kind: "START" | "NEXT"): Command => {
  ...
  return kind === "START"
    ? { kind, meta, schemaHash: hashSchema(schema) }
    : { kind, meta };
};
...
describe("composed schema compatibility", () => {
  it("runs through the ordinary session and restores exactly through replay", () => {
    ...
    const transported = JSON.parse(
      JSON.stringify(created.value.getEvents()),
    ) as readonly Event[];
    ...
    expect(restored.ok && restored.value.getSnapshot()).toEqual(
      created.value.getSnapshot(),
    );
  });
});
