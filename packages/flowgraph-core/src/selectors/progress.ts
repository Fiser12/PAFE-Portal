...
import type { FlowState, Progress } from "../domain/state";
...
const longestRemaining = (
  ...
  return node.edges.length === 0
    ...
    : 1 +
        Math.max(
          ...node.edges.map((edge) =>
            longestRemaining(schema, edge.to, [...visiting, nodeId]),
          ),
        );
};
...
export const progress = (schema: FlowSchema, state: FlowState): Progress => {
  ...
  const maximumRemainingEdges =
    state.status === "finished"
      ? 0
      : longestRemaining(schema, state.trail.at(-1) ?? schema.entry);
