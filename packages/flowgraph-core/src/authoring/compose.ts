...
export const namespaceNodeId = (
  instance: PackInstanceId,
  local: NodeId,
): NodeId => toNodeId(encoded("n:", instance, local));

export const namespaceQuestionId = (
  instance: PackInstanceId,
  local: QuestionId,
): QuestionId => toQuestionId(encoded("q:", instance, local));

export const namespaceOptionId = (
  instance: PackInstanceId,
  local: OptionId,
): OptionId => toOptionId(encoded("o:", instance, local));

export const namespaceOutcomeId = (
  instance: PackInstanceId,
  local: OutcomeId,
): OutcomeId => toOutcomeId(encoded("r:", instance, local));
...
export const checkPack = (
  pack: FlowPack,
  plugins: QuestionPluginRegistry,
): readonly PackProblem[] => {
  const problems: PackProblem[] = [];
  const ports = new Map<string, "entry" | "outlet">();
  const entries = new Map(
    pack.entries.map((entry) => [entry.id, entry] as const),
  );
  ...
  pack.entries.forEach((entry, index) => {
    ...
    if (first !== undefined) {
      problems.push(
        packProblem("duplicate-port", { port: entry.id, entry: index, first }),
      );
    } else {
    ...
    if (pack.nodes[entry.node]?.kind !== "page") {
      problems.push(
        packProblem("unknown-entry-node", { port: entry.id, node: entry.node }),
      );
    }
  });
  pack.outlets.forEach((outlet, index) => {
    ...
    if (pack.nodes[outlet.from]?.kind !== "page") {
      problems.push(
        packProblem("outlet-not-page", { port: outlet.id, node: outlet.from }),
      );
    }
  });
  ...
  const validEntries = [...new Set(pack.entries)]
    ...
    .filter((node, index, values) => {
      return (
        pack.nodes[node]?.kind === "page" && values.indexOf(node) === index
      );
    });
  ...
  syntheticNodes[superEntry] = {
    kind: "page",
    questions: [],
    edges: validEntries.map((to) => ({
      to,
      when: { kind: "any", values: [] },
    })),
  };
  pack.outlets.forEach((outlet, index) => {
    const source = syntheticNodes[outlet.from];
    if (source?.kind !== "page") return;
    const terminal = uniqueNodeId(
      { ...pack, nodes: syntheticNodes },
      `outlet_${String(index)}`,
    );
  ...
  check(synthetic, plugins)
    .filter(
      ({ severity, where }) =>
        severity === "error" && where.node !== superEntry,
    )
    .forEach((schemaIssue) => {
      problems.push(
        packProblem(
...
const rewriteGuard = (instance: PackInstanceId, guard: Guard): Guard => {
  switch (guard.kind) {
    ...
    case "any":
      return {
        ...guard,
        values: guard.values.map((value) => rewriteGuard(instance, value)),
      };
    case "cmp":
      return {
        ...guard,
...
const rewriteQuestion = (
  ...
  const namespaced = plugin?.namespace?.(question, {
    questionId: (id) => namespaceQuestionId(instance, id),
    optionId: (id) => namespaceOptionId(instance, id),
  }) ?? { ...question, id: namespaceQuestionId(instance, question.id) };
  return {
    ...namespaced,
  ...
  node.kind === "terminal"
    ...
    : {
        ...
        questions: node.questions.map((question) =>
          rewriteQuestion(instance, question, plugins),
        ),
...
export const compileComposition = (
  ...
  const instances = new Map<
    PackInstanceId,
    FlowComposition["instances"][number]
  >();
  ...
  composition.instances.forEach((instance, index) => {
    if (instances.has(instance.id)) {
      problems.push(
        compositionProblem("duplicate-instance", {
    ...
    if (packProblems === undefined) {
      packProblems = checkPack(instance.pack, plugins);
  ...
  composition.connections.forEach((connection, index) => {
    ...
    if (sourceInstance === undefined) {
      ...
      const outlet = sourceInstance.pack.outlets.find(
        ({ id }) => id === connection.from.outlet,
      );
      if (outlet === undefined) {
        problems.push(
  ...
  instances.forEach(({ id, pack }) => {
    pack.outlets.forEach((outlet) => {
      if (
        ...
        !bound.has(bindingKey({ instance: id, outlet: outlet.id }))
      ) {
        problems.push(
          compositionProblem("unconnected-outlet", {
            instance: id,
            port: outlet.id,
          }),
        );
  ...
  instances.forEach(({ id, pack }) => {
    Object.entries(pack.nodes).forEach(([local, node]) => {
      nodes[namespaceNodeId(id, toNodeId(local))] = rewriteNode(
        id,
        node,
        plugins,
      );
  ...
  validConnections.forEach(
    ({ sourceInstance, targetInstance, outlet, target }) => {
  ...
  const invalid = check(schema, plugins).filter(
