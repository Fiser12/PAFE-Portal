...
import type { GoldenProblem } from "../domain/problem";
...
import type { initialState } from "../engine/initial-state";
...
import { calculateCoverage, type CoverageCaseLog } from "./coverage";
...
const goldenSuiteSchema = z
  ...
  .superRefine(({ cases }, context) => {
    if (new Set(cases.map(({ id }) => id)).size !== cases.length) {
      context.addIssue({
        code: "custom",
        message: "Golden case ids must be unique",
      });
    }
  });
                ...
                ...(testCase.expect.state.activeAnswers
                  ? {
                      activeAnswers: Object.fromEntries(
                        Object.entries(testCase.expect.state.activeAnswers).map(
                          ([id, value]) => [
                            toQuestionId(id),
...
export const runGoldens = (
  ...
  if (
    suite.schema.id !== schema.id ||
    suite.schema.version !== schema.version
  ) {
    return err([{ code: "schema-identity-mismatch" }]);
  }
  const structural = check(schema, plugins).filter(
  ...
  for (const testCase of suite.cases) {
    ...
    for (const [step, intent] of testCase.commands.entries()) {
      const restored = replay(schema, events, plugins) as {
        readonly ok: true;
      ...
      const decided = decide(
        schema,
        restored.value,
        materialize(schema, intent, step),
        plugins,
      );
      if (!decided.ok) {
        return err([
    ...
    const actualActive = activeAnswers(schema, restored.value, plugins);
    if (
  ...
  return ok({
    ...
    coverage: calculateCoverage(schema, logs, plugins),
  });
};
