...
describe("check", () => {
  it.each(
    corpus.cases.filter(
      ({ expected }) =>
        !expected.startsWith("probe-") && expected !== "semantic-dead-end",
    ),
  )("detects $id as $expected", ({ schema, expected }) => {
    const problems = check(schema as unknown as FlowSchema);
    expect(problems.map(({ code }) => code)).toContain(
      expected as SchemaProblemCode,
    );
    expect(
      problems
        .filter(({ severity }) => severity === "error")
        .every(({ suggestion }) => suggestion),
    ).toBe(true);
  });
          ...
          questions: [
            {
              ...
              visibleWhen: {
                kind: "answered",
                q: toQuestionId("ancestor-number"),
              },
            },
            {
              kind: "text",
