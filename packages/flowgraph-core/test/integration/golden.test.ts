...
describe("runGoldens", () => {
  it.each([
    ...
    "executes %s JSON cases through replay and measures every edge",
    (_name, schema, suite) => {
      ...
      if (result.ok) {
        ...
        expect(result.value.coverage.edges.every(({ hits }) => hits > 0)).toBe(
          true,
        );
      }
    },
  ...
  it("reports identity, rejected-command, and projection failures explicitly", () => {
    expect(
      ...
      ok: false,
      error: [{ code: "schema-identity-mismatch" }],
    });
    ...
    expect(
      runGoldens(phq2Schema, rejected as unknown as GoldenSuiteV1),
    ).toMatchObject({
      ok: false,
    ...
    expect(
      runGoldens(phq2Schema, mismatch as unknown as GoldenSuiteV1),
    ).toMatchObject({
      ok: false,
  ...
  it("rejects duplicate case ids and structurally invalid schemas", () => {
    ...
    expect(
      runGoldens(invalid, phq2Goldens as unknown as GoldenSuiteV1),
    ).toMatchObject({
      ok: false,
  ...
  it("compares optional trail and active-answer projections deeply", () => {
    ...
    expect(
      runGoldens(phq2Schema, trailMismatch as unknown as GoldenSuiteV1),
    ).toMatchObject({
      ok: false,
    ...
    expect(
      runGoldens(phq2Schema, answerMismatch as unknown as GoldenSuiteV1),
    ).toMatchObject({
      ok: false,
      error: [{ code: "expectation-mismatch" }],
    });
