...
const record = (
  ...
  if (!decided.ok)
    throw new Error(`Rejected test command: ${String(decided.error[0]?.code)}`);
...
describe("replay", () => {
  it("restores a serialized mixed-source log and every derived projection exactly", () => {
    ...
    [state, events] = record(
      ...
      events,
      command("START", {
        schemaHash: hashSchema(retailSchema),
        meta: meta(1, "human"),
      }),
    );
    [state, events] = record(
    ...
    if (restored.ok) {
      expect(currentNode(retailSchema, restored.value)).toEqual(
        currentNode(retailSchema, state),
      );
  ...
  it("requires a matching first start and rejects duplicate starts", () => {
    ...
    expect(
      replay(schema, [{ ...start, schemaHash: "b".repeat(64) } as Event]),
    ).toEqual({
      ok: false,
  ...
  it("rejects graph transitions, invalid backs, and events after completion", () => {
    ...
    expect(
      replay(schema, [
        start,
        event("WENT_BACK", { from: toNodeId("page"), to: toNodeId("page") }),
      ]),
    ).toEqual({ ok: false, error: { code: "log-schema-mismatch" } });
    expect(
      replay(schema, [
        start,
        event("ANSWERED", { q: toQuestionId("missing"), value: "invalid" }),
      ]),
    ).toEqual({ ok: false, error: { code: "log-schema-mismatch" } });
  ...
  it("requires an adjacent, metadata-identical finish after a terminal advance", () => {
    ...
    const next = decide(
      schema,
      state,
      command("NEXT", { meta: meta(9, "agent") }),
    );
    if (!next.ok || next.value.length !== 2)
    ...
    expect(
      replay(schema, [start, advanced, { ...finished, at: meta(10).at }]),
    ).toEqual({
      ok: false,
      error: { code: "log-schema-mismatch" },
    });
