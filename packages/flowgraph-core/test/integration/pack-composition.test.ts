...
const item = <Value>(values: readonly Value[], index: number): Value => {
  ...
  if (value === undefined)
    throw new Error(`missing fixture item ${String(index)}`);
  return value;
};
...
describe("schema pack composition", () => {
  ...
  it("compiles to one ordinary schema and rewrites every identity and reference kind", () => {
    ...
    expect(parseSchema(JSON.parse(JSON.stringify(schema)))).toEqual({
      ok: true,
      value: schema,
    });
    expect(
      check(schema).filter(({ severity }) => severity === "error"),
    ).toEqual([]);
    expect(
      probe(schema).pages.every(({ deadEndsFound }) => deadEndsFound === 0),
    ).toBe(true);
    ...
    expect(serializedGuard).toContain(
      namespaceQuestionId(instance, toQuestionId("age")),
    );
    expect(serializedGuard).toContain(
      namespaceQuestionId(instance, toQuestionId("choice")),
    ...
    const accepted =
      schema.nodes[namespaceNodeId(instance, toNodeId("accepted"))];
    expect(accepted?.kind).toBe("page");
    if (accepted?.kind === "page") {
      expect(accepted.edges).toHaveLength(2);
      expect(accepted.edges[0]?.to).toBe(
        namespaceNodeId(instance, toNodeId("exceptional")),
      );
    ...
    const exceptional =
      schema.nodes[namespaceNodeId(instance, toNodeId("exceptional"))];
    expect(exceptional).toEqual({
      kind: "terminal",
      outcome: namespaceOutcomeId(instance, toOutcomeId("exceptional")),
    });
    expect(JSON.stringify(schema)).not.toMatch(
      /"pack"|"instances"|"connections"|"outlets"/u,
    );
  });
  ...
  it("uses an injective namespace even when ids contain separators", () => {
    ...
    expect(
      namespaceQuestionId(toPackInstanceId("x"), toQuestionId("y")),
    ).not.toBe(namespaceOptionId(toPackInstanceId("x"), toOptionId("y")));
  });
  ...
  it("executes compiled output through ordinary goldens with full edge coverage", () => {
    ...
    const option = (id: "yes" | "no") =>
      namespaceOptionId(instance, toOptionId(id));
      ...
      cases: [
        {
          ...
          expect: {
            outcome: namespaceOutcomeId(
              toPackInstanceId("terminal"),
              toOutcomeId("submitted"),
            ),
          },
        },
        {
  ...
  it("preserves concrete factory configuration across repeated pack instances", () => {
    ...
    const firstPage =
      schema.nodes[
        namespaceNodeId(toPackInstanceId("first"), toNodeId("start"))
      ];
    const secondPage =
      schema.nodes[
        namespaceNodeId(toPackInstanceId("second"), toNodeId("start"))
      ];
    expect(firstPage?.kind === "page" && firstPage.title?.fallback).toBe(
      "First",
    );
    expect(secondPage?.kind === "page" && secondPage.title?.fallback).toBe(
      "Second",
    );
    expect(
      firstPage?.kind === "page" &&
  ...
  it("compiles 20 packs and 500 private nodes in under one second", () => {
    ...
    const ids = Array.from({ length: 20 }, (_, index) =>
      toPackInstanceId(`large-${String(index)}`),
    );
    const composition: FlowComposition = {
  ...
  it.each([
    [
      ...
      () => {
        ...
        return {
          ...base,
          instances: [...base.instances, item(base.instances, 0)],
        };
      },
    ],
    [
    ...
    [
      ...
      () => {
        ...
        return {
          ...base,
          connections: [...base.connections, item(base.connections, 0)],
        };
      },
    ],
    [
      ...
      () => ({
        ...validComposition(),
        connections: validComposition().connections.slice(0, 1),
      }),
    ],
    [
      "invalid-pack",
      () => {
        ...
        return {
          ...base,
          instances: [
            { ...first, pack: { ...first.pack, entries: [] } },
            item(base.instances, 1),
          ],
        };
      },
    ],
    ...
    if (!result.ok)
      expect(result.error.map(({ code: actual }) => actual)).toContain(code);
  });
