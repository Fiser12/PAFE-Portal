...
describe("numeric and guard evaluation", () => {
  it("evaluates num, answer, score, sum, and all comparison operators exactly", () => {
    ...
    expect(evaluateNumeric(schema, state, sum)).toEqual({
      kind: "known",
      value: 6,
    });
  ...
  it("treats absent weights as zero and unknown selected ids or sum operands as unknown", () => {
    ...
    const choice = page.questions.find(
      ({ id }) => id === toQuestionId("choice"),
    );
    ...
    expect(
      evaluateNumeric(
        unweighted,
        stateOn(unweighted, "page", { choice: [toOptionId("zero")] }),
        {
          kind: "score",
          q: toQuestionId("choice"),
        },
    ...
    expect(
      evaluateNumeric(
        schema,
        stateOn(schema, "page", { choice: [toOptionId("missing")] }),
        {
          kind: "score",
          q: toQuestionId("choice"),
        },
    ...
    expect(
      evaluateNumeric(
        schema,
        stateOn(schema, "page", { count: toSafeInt(1) }),
        {
          kind: "score",
          q: toQuestionId("count"),
        },
