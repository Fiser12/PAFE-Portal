...
import { parseSchema, toNodeId } from "../support/runtime.js";
...
describe("parseSchema", () => {
  ...
  it("normalizes negative zero throughout numeric wire values", () => {
    ...
    if (result.ok) {
      ...
      if (page?.kind === "page") {
        expect(
          Object.is(
            page.questions[0]?.kind === "number" && page.questions[0].min,
            -0,
          ),
        ).toBe(false);
      }
  ...
  it("rejects question kinds that have no registered plugin", () => {
    expect(
      parseSchema({
        ...validSchema,
        nodes: {
          ...
          page: {
            ...
            questions: [
              { kind: "attachment", id: "photos", text: { key: "photos" } },
            ],
          },
        },
      }).ok,
  ...
  it("accepts an exact plugin manifest and rejects missing or incompatible versions", () => {
    expect(
      parseSchema({
        ...validSchema,
        questionPlugins: { number: "test", select: "test" },
      }).ok,
    ).toBe(true);
    expect(
      parseSchema({
        ...validSchema,
        questionPlugins: { number: "old", select: "test" },
      }).ok,
    ).toBe(false);
    expect(
      parseSchema({ ...validSchema, questionPlugins: { number: "test" } }).ok,
    ).toBe(false);
  });
  ...
  it.each([
    ...
    [
      ...
      {
        ...validSchema,
        nodes: { ...validSchema.nodes, done: { kind: "mystery" } },
      },
    ],
    [
      "unknown question field",
      {
        ...
        nodes: {
          ...
          page: {
            ...validSchema.nodes.page,
            questions: [
              { ...validSchema.nodes.page.questions[0], extra: true },
            ],
          },
        },
      },
      ...
      "unsafe numeric bound",
      {
        ...
        nodes: {
          ...
          page: {
            ...
            questions: [
              {
                ...validSchema.nodes.page.questions[0],
