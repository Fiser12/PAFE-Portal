...
const start = (schema: FlowSchema): Command =>
...
describe("decide", () => {
  it("requires one explicit START with the matching schema hash", () => {
    ...
    expect(
      decide(schema, state, command("START", { schemaHash: "b".repeat(64) })),
    ).toEqual({
      ok: false,
  ...
  it("returns exact answer/navigation problems with no events", () => {
    ...
    expect(
      decide(schema, state, command("ANSWER", { q: toQuestionId("missing") })),
    ).toEqual({
      ok: false,
    ...
    if (!next.ok)
      expect(next.error.map((problem) => problem.code)).toContain("required");
  });
  ...
  it("records structurally valid semantic-invalid answers", () => {
    ...
    const result = decide(
      schema,
      state,
      command("ANSWER", { q: toQuestionId("age"), value: 130 }),
    );

    expect(result.ok).toBe(true);
    if (result.ok)
      expect(result.value[0]).toMatchObject({ kind: "ANSWERED", value: 130 });
  });
  ...
  it("navigates zero-question pages, supports first-page BACK, and finishes atomically", () => {
    ...
    expect(decide(schema, state, command("BACK"))).toEqual({
      ok: true,
      value: [],
    });
    const result = decide(
      schema,
      state,
      command("NEXT", { meta: meta(7, "agent") }),
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
      ...
      expect(decide(schema, finished, command("NEXT"))).toEqual({
        ok: true,
        value: [],
      });
  ...
  it("records BACK to the previous trail node and reports no edge or missing nodes", () => {
    ...
    expect(
      decide(schema, { ...started(schema), trail: [] }, command("NEXT")),
    ).toEqual({
      ok: false,
    ...
    expect(
      decide(
        schema,
        { ...started(schema), trail: [toNodeId("done")] },
        command("NEXT"),
      ),
    ).toEqual({
      ok: false,
