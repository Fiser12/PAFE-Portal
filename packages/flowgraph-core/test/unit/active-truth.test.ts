...
describe("visibility and active truth", () => {
  it("evaluates same-page visibility in document order", () => {
    ...
    const visible = stateOnTrail(schema, ["choice"], {
      route: [toOptionId("a")],
      detail: "kept",
    });
  ...
  it("deactivates abandoned-route answers while retaining stored facts", () => {
    ...
    expect(storedAnswer(abandoned, toQuestionId("detail"))).toBe(
      "from branch a",
    );
  ...
  it("reactivates the latest retained answer only after re-entering its page", () => {
    ...
    expect(activeAnswers(schema, returned)[toQuestionId("detail")]).toBe(
      "prefilled",
    );
  ...
  it("covers recursive guard composition, missing questions, and non-page visibility", () => {
    const schema = branchSchema();
    const state = stateOnTrail(schema, ["choice"], {
      route: [toOptionId("a")],
    });
    const selected = {
    ...
    expect(evaluateGuard(schema, state, { kind: "not", value: selected })).toBe(
      "false",
    );
    expect(
      evaluateGuard(schema, state, { kind: "all", values: [selected] }),
    ).toBe("true");
    expect(
      evaluateGuard(schema, state, { kind: "any", values: [selected] }),
    ).toBe("true");
    expect(
      evaluateGuard(schema, state, {
        kind: "answered",
        q: toQuestionId("missing"),
      }),
    ).toBe("false");
    expect(visibleQuestions(schema, state).map(({ id }) => id)).toEqual([
      "route",
      "detail",
    ]);
    expect(
      visibleQuestions(schema, stateOnTrail(schema, ["done"], {})),
    ).toEqual([]);
