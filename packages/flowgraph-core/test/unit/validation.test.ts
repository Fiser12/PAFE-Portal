...
describe("answer validation", () => {
  it.each([
    ...
    expect(
      structuralAnswerProblems(schema, state, q, value).map(
  ...
  it("records semantic violations and surfaces them through selectors", () => {
    ...
    expect(
      questionProblems(schema, state, toQuestionId("notes")).map((p) => p.code),
    ).toEqual(["too-long"]);
    expect(
      questionProblems(schema, state, toQuestionId("age")).map((p) => p.code),
    ).toEqual(["out-of-range"]);
    expect(
      questionProblems(schema, state, toQuestionId("single")).map(
        (p) => p.code,
      ),
    ).toEqual(["required"]);
    ...
    expect(
      questionProblems(schema, stateOn(schema, "done"), toQuestionId("age")),
    ).toEqual([]);
  });
});
